...
import { ArrowRight, ArrowDown, ChartLineUp, Clock, MapPin, UsersThree } from "@phosphor-icons/react/dist/ssr";
...
export default async function HomePage() {
  ...
  return <>
    ...
    <FeatureBand>
      <FeatureItem icon={<UsersThree size={32} weight="duotone" />} title="Persönliche Gespräche" text="Berufe werden im direkten Austausch greifbar." />
      <FeatureItem icon={<Clock size={32} weight="duotone" />} title="Kurze Wege" text="Gemeinsam geplant und direkt in der Schule umgesetzt." />
      <FeatureItem icon={<MapPin size={32} weight="duotone" />} title="Starke Region" text="Fürth, Nürnberg, Erlangen und Umgebung." />
      <FeatureItem icon={<ChartLineUp size={32} weight="duotone" />} title="Mehr Chancen" text="Kontakte, die über den Tag hinaus wirken können." />
    </FeatureBand>
    ...
    <section className="process-section"><div className="page-shell"><div className="process-intro"><p className="eyebrow">So läuft es ab</p><h2 className="display">Einfach. Effizient. Persönlich.</h2><p>Von der gemeinsamen Planung bis zum nächsten Kontakt bleibt der Ablauf klar und überschaubar.</p></div><ProcessSteps /></div></section>
