...
export default async function ContactPage({ searchParams }: { searchParams: Promise<{ rolle?: string }> }) {
  ...
  return <>
    <section className="bg-white py-14 sm:py-20"><div className="page-shell grid gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-start"><div><p className="eyebrow">Kontakt</p><h1 className="display mt-4 text-5xl font-extrabold text-brand-navy sm:text-6xl">Wir freuen uns auf Ihre Nachricht.</h1><p className="mt-6 max-w-xl text-xl leading-relaxed text-brand-ink/75">Sie möchten als Schule oder Unternehmen dabei sein? Schreiben Sie uns kurz, was Sie vorhaben. Wir melden uns persönlich.</p><div className="mt-10 grid gap-5 text-brand-ink/80"><p className="flex gap-3"><EnvelopeSimple size={25} className="mt-0.5 text-brand-green" /><span><strong className="block text-brand-navy">E-Mail</strong><ContactDetail kind="email" value={content.site_info.contact_email} /></span></p><p className="flex gap-3"><Phone size={25} className="mt-0.5 text-brand-green" /><span><strong className="block text-brand-navy">Telefon</strong><ContactDetail kind="phone" value={content.site_info.contact_phone} /></span></p><p className="flex gap-3"><MapPin size={25} className="mt-0.5 text-brand-green" /><span><strong className="block text-brand-navy">Region</strong>{content.site_info.address}</span></p></div><EncounterImage label="Persönlich erreichbar in der Region" className="mt-10 min-h-[14rem]" /></div><ContactForm key={initialRole} initialRole={initialRole} token={process.env.ATONDIX_STUDIO_SITE_TOKEN} /></div></section>
