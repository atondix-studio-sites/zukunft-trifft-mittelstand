...
export function AudienceCard({ kind }: { kind: "schule" | "unternehmen" }) {
  ...
  return <article className="surface-card group flex h-full flex-col p-6 transition hover:-translate-y-1 sm:p-8">
    ...
    <Link href={`/kontakt?rolle=${kind}`} className="mt-7 inline-flex min-h-11 items-center gap-2 font-bold text-brand-green transition group-hover:gap-3">{school ? "Als Schule anfragen" : "Als Unternehmen teilnehmen"} <ArrowRight size={20} /></Link>
  </article>;
}
